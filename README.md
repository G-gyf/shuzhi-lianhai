...
├─ tests/               # 验收测试（unittest，129 用例：口径/工具/分析/对话/地区隔离/国别与供应链/冷启动容错回归）
...
```bash
python -m unittest discover -s tests -t .   # 单元/验收测试：129 项通过
python smoke_test.py                        # 冒烟测试：60 项通过
```

- 单测覆盖：口径与年份、工具与产品卡、分析校验与保存、SSE 对话网关、地区隔离与资料源、
