> 当前口径以 [本次更新说明](docs/当前口径与实施更新_20260925.md) 为准。Coze云端待实施；现有对话/光球和本地规则为已实现的接入基础。
...
浏览器打开 http://127.0.0.1:8000 → 右下角「AI 助手」打开对话工作台。
...
├─ kb/kb-sc-2023.sqlite # CSMAR 供应链：top5/集中度/二跳链
...
├─ tests/               # 验收测试（unittest，38 用例：口径/工具/分析/对话/地区隔离）
...
- 已接入匿名前五大客户/供应商及集中度。匿名对象不跨企业拼接；有限来源网络不代表全量供应链。银行实名关系与 Neo4j 仅为设计预留，本项目不实现。
...
5. Neo4j：系统设计预留，待银行提供有权限的实名数据；本项目不实现；
6. 匿名供应链只作局部结构画像，不建设全量供应链；
7. 正式身份系统替代演示身份；真实分行资料替代合成示范资料（M4 边界）。
