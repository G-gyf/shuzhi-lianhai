...
| 7 | 实控人边 / 全量供应链边 / 多跳关联查询 | P1/P2 扩展 | ⚠️ v1.4 已接入 CSMAR 前五大客户/供应商量化边（单跳画像）；**多跳关联查询已从本期能力移除**，与银行授权实名关联一并通过 Neo4j 扩展设计预留，不属本项目实施范围 |
...
- **供应链接口升级**：`supply_chain()` 输出结构化量化边（CSMAR 前五大：名称+排名+金额+占比，BUYS_FROM 供应商边自 v1.4 起为真实数据）+ 文本具名锚点边 + 子公司国家边（as-of）；**只呈现单跳画像**，不推导传导结论。
...
- **多跳关联查询（本次调整）**：前端「二跳传导链」区块与 `sc_of()`/`supply_chain()` 的多跳字段已一并移除。原实现有两处问题：一是 `network` 表 95 行中仅 2 行两跳齐全（均 2019 年），不足以支撑展示；二是第二跳缺失时对 NaN 取整导致 16 家企业的供应链接口返回 500。现统一改为**单跳画像**，多跳归入 Neo4j 扩展设计，不作为本期能力输出。
...
- **演示身份**：`demo-token-region-a` / `demo-token-region-b`（Bearer 令牌，服务端解析辖区）。**正式身份系统不属本项目实现目标**：`users.identity_mode` 与 `server/runtime.py::authenticate()` 为行内统一认证预留接入位，投产时由行内身份系统替换，本项目只描述该接入点。
...
- `smoke_test.py` first 口径注记断言已同步（本次），冒烟 60 项全通过；单元测试现为 115 项。
...
- 生成式引擎为**扣子编程项目部署后的正式服务地址**（`LANGGRAPH_BASE_URL=https://kmj3bsz4kj.coze.site`），非本地/沙箱实例。演示前仍建议先发一次预热请求，并以 `/api/health` 的 `engine.reachable` 确认真实可连通。
