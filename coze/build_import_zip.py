# -*- coding: utf-8 -*-
"""生成 coze.cn 可导入的工作流 ZIP 包（含阶梯测试包）。

用途：把「数智链海出海助手」工作流的骨架（开始节点变量、大模型节点提示词、
插件节点参数、结束节点输出）打包成 coze.cn「导入工作流」能识别的 ZIP，
省去在界面上逐个建节点、逐个粘提示词的工作。

同时产出**阶梯测试包**，用于定位导入失败是"入口/格式"问题还是"某个节点"问题：
    nano  —— 开始 + 结束（2 节点）：只验证 ZIP/manifest/start/end 格式
    smoke —— 开始 + 大模型 + 结束（3 节点）：再验证大模型节点格式
    full  —— 完整 7 节点主链路

格式依据（公开资料核对于 2026-09-25，见 docs 与 coze/workflow_design.md）：
...
导入后仍需在界面手工完成（ID 属于各自工作空间，无法预置）：
1. 给插件节点选择工作空间里的插件与工具；2. 给大模型节点选择模型。

用法：
    python -X utf8 coze/build_import_zip.py            # 生成三个变体到 coze/import/
...
ICONS = "https://lf3-static.bytednsdoc.com/obj/eden-cn/dvsmryvd_avi_dvsm/ljhwZthlaukjlkulzlp/icon/"
ICON_START = ICONS + "icon-Start-v2.jpg"
ICON_LLM = ICONS + "icon-LLM-v2.jpg"
ICON_END = ICONS + "icon-End-v2.jpg"
ICON_PLUGIN = ICONS + "icon-Plugin-v2.jpg"
...
def _assemble(nodes: list[str], edges: list[str]) -> str:
    return (
...
def build_nano_yaml() -> str:
    """阶梯第 1 级：开始 + 结束（验证 ZIP/manifest/start/end）。"""
    return _assemble(
        [start_node(), end_node("100001", x=200, y=0)],
        ['    - source_node: "100001"\n      target_node: "900001"'],
    )


def build_smoke_yaml() -> str:
    """阶梯第 2 级：开始 → 大模型 → 结束（再验证大模型节点）。"""
    return _assemble(
        [start_node(),
         llm_node("200001", "冒烟-大模型", "你是连通性测试助手：原样返回用户输入。",
                  "{{message}}", [("message", "100001", "message")], x=-600, y=0),
         end_node("200001", x=200, y=0)],
        ['    - source_node: "100001"\n      target_node: "200001"',
         '    - source_node: "200001"\n      target_node: "900001"'],
    )
...
def build_workflow_yaml() -> str:
    ...
    nodes = [
        start_node(),
        llm_node("200001", "N02-意图与参数", load_prompt("intent.md"),
    ...
    return _assemble(nodes, edges)


VARIANTS = {"nano": build_nano_yaml, "smoke": build_smoke_yaml, "full": build_workflow_yaml}
...
def check_zip(path: Path, yaml_body: str) -> list[str]:
    ...
    with zipfile.ZipFile(path) as z:
        names = z.namelist()
        root = f"Workflow-{NAME}-draft-0001"
        if names[0] != f"{root}/MANIFEST.yml":
            issues.append(f"第一个条目必须是 MANIFEST.yml，实际：{names[0]}")
        if len(names) != 2:
            issues.append(f"应只有 2 个条目（不含目录条目），实际：{names}")
        if f"{root}/workflow/{NAME}-draft.yaml" not in names:
    ...
    while True:
        ...
        off = idx + 30 + struct.unpack_from("<H", raw, idx + 26)[0]
    while True:
        idx = raw.find(b"PK\x01\x02", off)
        if idx == -1:
            break
        if struct.unpack_from("<H", raw, idx + 4)[0] != 20:
            issues.append("CD vmade 应为 20")
        if struct.unpack_from("<I", raw, idx + 38)[0] != 0:
            issues.append("CD external_attr 应为 0")
        off = idx + 46 + struct.unpack_from("<H", raw, idx + 28)[0]
    try:
        import yaml
...
def main() -> int:
    ap = argparse.ArgumentParser(description="生成 coze.cn 可导入的工作流 ZIP（含阶梯测试包）")
    ap.add_argument("--check", action="store_true", help="生成后校验结构与字节契约")
    ap.add_argument("--only", choices=sorted(VARIANTS), help="只生成某个变体")
    args = ap.parse_args()

    targets = [args.only] if args.only else ["nano", "smoke", "full"]
    for variant in targets:
        body = VARIANTS[variant]()
        stem = f"Workflow-{NAME}"
        if variant != "full":
            stem += f"_{variant}"
        out = OUT_DIR / f"{stem}-draft-0001.zip"
        write_zip(out, [
            (f"Workflow-{NAME}-draft-0001/MANIFEST.yml", build_manifest().encode("utf-8")),
            (f"Workflow-{NAME}-draft-0001/workflow/{NAME}-draft.yaml", body.encode("utf-8")),
        ])
        out.with_suffix(".yaml").write_text(body, encoding="utf-8")
        import yaml
        data = yaml.safe_load(body)
        kinds: dict[str, int] = {}
        for n in data["nodes"]:
            kinds[n["type"]] = kinds.get(n["type"], 0) + 1
        print(f"[生成] {out.name}  ({out.stat().st_size} 字节)  "
              f"节点 {len(data['nodes'])} 个 {json.dumps(kinds)}  连线 {len(data['edges'])} 条")
        if args.check:
            issues = check_zip(out, body)
            if issues:
                print("   [校验] ❌ " + "；".join(issues))
                return 1
            print("   [校验] ✅ 结构与字节契约通过")
    return 0
